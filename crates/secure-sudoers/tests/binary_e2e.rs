...
/// Minimal policy with no tools — any tool invocation will be denied.
...
/// Pointing SECURE_SUDOERS_POLICY_PATH at a missing file must exit fatally
#[test]
fn test_binary_missing_policy_exits_fatally() {
    ss_cmd()
        .env("SECURE_SUDOERS_POLICY_PATH", "/nonexistent/path/policy.json")
...
/// A policy file whose `.sig` is garbage must exit fatally
#[test]
fn test_binary_bad_signature_exits_fatally() {
    ...
    // Write a valid policy JSON but a corrupted 64-byte signature.
    let policy_path = dir.path().join("policy.json");
    ...
    drop(sk); // key not used for signing here
...
/// A valid policy that contains no tools: any tool is denied
#[test]
...
/// Same as above but invoked as `secure-sudoers nonexistent_tool`
#[test]
