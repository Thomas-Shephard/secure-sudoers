...
pub(crate) fn load_policy_with_pubkey(
    ...
    let policy_bytes =
        std::fs::read(path).map_err(|e| Error::IoContext(format!("Failed to read policy {path}"), e))?;
    ...
    let pubkey_arr: [u8; 32] = pubkey_bytes
        .as_slice()
        .try_into()
        .map_err(|_| Error::Validation("Integrity failure: public key must be 32 bytes".to_string()))?;
...
mod tests {
    ...
    fn sudo_command_mismatch_detected() {
        ...
        assert!(result.unwrap_err().to_string().contains("Spoofing attempt detected"));
    }
    ...
    fn test_load_policy_missing_file() {
        ...
        assert!(result.is_err());
        assert!(
            result.unwrap_err().to_string().contains("Failed to read policy"),
