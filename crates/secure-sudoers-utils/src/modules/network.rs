...
pub fn run(url: &str, pubkey_path: &str) -> Result<(), String> {
    ...
    let mut tmp_sig = tempfile::NamedTempFile::new_in(policy_dir).map_err(|e| {
        format!(
            "Cannot create temp sig file in {}: {e}",
            policy_dir.display()
        )
    })?;
    tmp_sig
    ...
    tmp_sig
        .flush()
        .map_err(|e| format!("Failed to flush temp sig file: {e}"))?;
    tmp_sig
        .persist(&sig_path)
        .map_err(|e| format!("Atomic rename of signature failed: {}", e.error))?;

    let mut tmp_policy = tempfile::NamedTempFile::new_in(policy_dir).map_err(|e| {
        format!(
            "Cannot create temp policy file in {}: {e}",
            policy_dir.display()
        )
    })?;
    tmp_policy
    ...
    tmp_policy
        .flush()
        .map_err(|e| format!("Failed to flush temp policy file: {e}"))?;
    tmp_policy
        .persist(POLICY_PATH)
        .map_err(|e| format!("Atomic rename of policy failed: {}", e.error))?;
