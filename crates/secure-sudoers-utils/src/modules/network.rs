...
pub fn run(url: &str, pubkey_path: &str) -> Result<(), Error> {
    ...
    let sig_arr: [u8; 64] = sig_bytes
        .as_slice()
        .try_into()
        .map_err(|_| Error::Validation(format!("Signature must be 64 bytes; got {}", sig_bytes.len())))?;
    verifying_key
    ...
    new_policy
        .validate()
        .map_err(|e| Error::Validation(format!("Downloaded policy failed semantic validation: {e}")))?;
    ...
    let mut tmp_sig = tempfile::NamedTempFile::new_in(policy_dir).map_err(|e| {
        Error::IoContext(format!("Cannot create temp sig file in {}", policy_dir.display()), e)
    })?;
    tmp_sig
        .write_all(&sig_bytes)
    ...
    let mut tmp_policy = tempfile::NamedTempFile::new_in(policy_dir).map_err(|e| {
        Error::IoContext(format!("Cannot create temp policy file in {}", policy_dir.display()), e)
    })?;
    tmp_policy
...
fn fetch_limited(
    ...
    if !response.status().is_success() {
        return Err(
            Error::Network(format!("HTTP {} for {url}", response.status())),
        );
    }
