...
pub fn cmd_gen_keys() -> Result<(), String> {
    ...
    write_key_file(PRIVATE_KEY_FILE, "SECURE SUDOERS PRIVATE KEY", &signing_key.to_bytes(), 0o600)?;
    write_key_file(PUBLIC_KEY_FILE, "SECURE SUDOERS PUBLIC KEY", &verifying_key.to_bytes(), 0o644)?;
...
pub fn write_key_file(path: &str, label: &str, bytes: &[u8], unix_mode: u32) -> Result<(), String> {
    ...
    f.set_permissions(perms).map_err(|e| format!("Failed to set permissions on {path}: {e}"))?;
    f.write_all(content.as_bytes()).map_err(|e| format!("Failed to write {path}: {e}"))?;
    Ok(())
}
