...
#[derive(Parser)]
#[command(name = "secure-sudoers-utils", about = "Policy key management and distribution for secure-sudoers")]
struct Cli {
    #[command(subcommand)]
    command: Commands,
}
...
enum Commands {
    ...
    Sign { policy_path: String, key_path: String },
...
fn cmd_sign(policy_path: &str, key_path: &str) -> Result<(), String> {
    ...
    let policy_bytes = std::fs::read(policy_path).map_err(|e| format!("Cannot read {policy_path}: {e}"))?;
    let signature = ed25519_dalek::Signer::sign(&signing_key, &policy_bytes);
    let sig_path = format!("{policy_path}.sig");
    std::fs::write(&sig_path, signature.to_bytes()).map_err(|e| format!("Failed to write {sig_path}: {e}"))?;
...
fn cmd_generate_man_page() -> Result<(), String> {
    ...
    man.render(&mut buffer).map_err(|e| format!("Failed to render man page: {e}"))?;
    std::io::Write::write_all(&mut std::io::stdout(), &buffer).map_err(|e| format!("Failed to write to stdout: {e}"))?;
    Ok(())
}
